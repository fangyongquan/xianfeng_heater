...
export default {
  ...
  mutations: { // 更改 Vuex 的 store 中的状态的唯一方法是提交 mutation, 只能同步
    ...
    updateDeviceStatus(state, payload) {
      ...
      this.commit('updateDeviceAttr', attr)
    ...
    updateDeviceAttr(state, attr = {}) {
      attr = Object.assign({}, state.attr, attr)
      attr.onlinestate = attr.onlinestate === 'online' || attr.onlinestate === true
      attr.powerstate = attr.onlinestate && (attr.powerstate === 1 || attr.powerstate === true)
      attr.OSD = attr.powerstate && (attr.OSD === 1 || attr.OSD === true)
      attr.childLockOnOff = attr.powerstate && (attr.childLockOnOff === 1 || attr.childLockOnOff === true)
